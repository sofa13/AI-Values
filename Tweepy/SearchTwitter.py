...
#searchQuery = 'from:jasonmillar filter:links'	# this is what we're searching for
searchQuery = 'robot privacy'
		...
		try:
			...
			for tweet in new_tweets:
				...
				try:
					...
					# url = urls[0].get("url", "[]")
					todump.append(tweet._json["text"])
					todump.append(tweet._json["entities"]["urls"])
					# todump.append(tweet._json["user"]["url"])
				except Exception as e:
					print(e)
					#pass
				else:
					# if text in textset or verified == "false":
						# pass
					# else:
						# textset.add(text)
						# print("%s: %s" % (text, url))
